...
# now_str = "2024-07-23_15-25-57-first_AUV"
...
line_vel_frente, = ax2.plot([], [], 'g-', label='Forward speed')
line_com_rot, = ax3.plot([], [], 'r-', label='Angular command')
line_com_frente, = ax2.plot([], [], 'b-', label='Forward command')

error_plot, = ax4.plot([], [], 'b-')

zero, = ax2.plot([-1000, 1000], [0, 0], ':', color='gray')
...
def init():
    ...
    error_plot.set_data([], [])
    return point_rob, point_target, line_vel_rot, line_vel_frente, zero, error_plot, line_com_frente, line_com_rot,
...
def update(frame):
    ...
    return point_rob, point_target, trajectory_rob, trajectory_target, line_vel_rot, line_vel_frente, error_plot
